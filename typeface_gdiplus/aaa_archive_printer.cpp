...
namespace typeface_gdiplus
{
   ...
   bool printer::open(const ::scoped_string & scopedstrDeviceName)
   {
      if (is_opened())
         close();
      if(!OpenPrinter((::i8 *)(const ::string &)pszDeviceName,&m_hPrinter,nullptr))
         return false;
   ...
   bool printer::document_properties::initialize(::typeface_gdiplus::printer * pprinter, DEVMODE * pdevmode)
   {
      ...
      if (m_hdc != nullptr)
         return false;
      ::i32 iSize = DocumentProperties(nullptr, pprinter->m_hPrinter, (::i8 *)(const ::string &)pprinter->m_strName, nullptr, nullptr, 0);
      m_pdevmode = (DEVMODE *)malloc(iSize);
      if (!DocumentProperties(nullptr, pprinter->m_hPrinter, (::i8 *)(const ::string &)pprinter->m_strName, m_pdevmode, nullptr, DM_OUT_BUFFER))
      {
