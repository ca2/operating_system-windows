...
namespace windowing_win32
{
   ...
   void graphics::initialize_graphics_graphics(::windowing::window * pwindow)
   {
      ...
      m_pwindowbuffer->initialize_window_buffer(pwindow);

      ::graphics::bitmap_source_buffer_graphics::initialize_graphics_graphics(pwindow);
      ...
      ::graphics::double_buffer_graphics::initialize_graphics_graphics(pwindow);
...
//      if (sizeImageBufferItem != pbufferitem->m_sizeBufferItem)
   ...
   bool graphics::update_buffer(::graphics::buffer_item * pbufferitem)
   {
         ...
         || pbufferitem->m_pimageBufferItem->m_sizeRaw != m_pwindow->m_sizeRaw)
      {
         ...
         ::cast < ::windowing_win32::layered_window_buffer > playeredwindowbuffer =
            m_pwindowbuffer;
...
//      }
   ...
   void graphics::destroy_buffer()
   {
      ...
      //   if (pbufferitem->m_pwindowbuffer)
      //   {

      //      pbufferitem->m_pwindowbuffer->destroy_buffer();

      //      pbufferitem->m_pwindowbuffer.release();
      ...
      double_buffer_graphics::destroy_buffer();
   ...
   void graphics::update_screen()
   {
      ...
      return double_buffer_graphics::update_screen();
   ...
   void graphics::_on_configure_window(::graphics::buffer_item * pbufferitem)
   {
      ...
      //                  hwndInsertAfter = (HWND) pwindow2->_HWND();
      ...
      //         bool bWindowVisible = ::IsWindowVisible((HWND) pwindow->_HWND()) ? true : false;

      //         bool bExTopMost =
      //            (::GetWindowLongPtr((HWND) pwindow->_HWND(), GWL_EXSTYLE) & WS_EX_TOPMOST) != 0;
   ...
   void graphics::on_update_screen(::graphics::buffer_item * pbufferitem)
   {
      ...
      // The buffer item is the frame being presented. Do not derive its rectangle
      // from window-wide cached state, which may already describe another frame.
      auto rectangleWindow = ::i32_rectangle(
         pbufferitem->m_pointBufferItem,
         pbufferitem->m_sizeBufferItem);

      auto rectangleRaw = ::i32_rectangle(m_pwindow->m_sizeRaw);

      auto rectangleWindowFixed = rectangleWindow.intersection(rectangleRaw);

      auto sizeWindowFixed = rectangleWindowFixed.size();
      ...
      if (playeredwindowbuffer && !m_papplication->m_gpu.m_bUseSwapChainWindow)
      {
         ...
         //if (pbufferitem->m_pimageBufferItem->on_host_read_pixels(ppixmapWindowBuffer))
         //{
      ...
      try
      {
         ...
         if (playeredwindowbuffer && !m_papplication->m_gpu.m_bUseSwapChainWindow)
         {
            ...
            if (sizeBufferItemWindowFixed != sizeBuffer)
            {
...
//ULW_ALPHA);
   ...
   void graphics::update_screen_task()
   {
