...
void input::defer_input()
{
   ...
   if (m_particleaMouseHandler.has_element())
   {
      ...
      if (!m_ptaskMouse)
      {
         ...
         ::mouse_hook::install(this);

         m_ptaskMouse = app_fork([this]()
            {

               at_end_of_scope
               {

                                 m_ptaskMouse.release();
   ...
   if (m_particleaKeyboardHandler.has_element())
   {
      if (!m_ptaskKeyboard)
      {
         ...
         ::keyboard_hook::install(this);

         m_ptaskKeyboard = app_fork([this]()
            {

               at_end_of_scope
               {

                                 m_ptaskKeyboard.release();
