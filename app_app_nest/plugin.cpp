...
/*   UINT __cdecl plugin::ThreadProcStart(LPVOID lpv)
   ...
   void plugin::run_start_install(const char * pszInstall)
   {
   
      {
         ...
         file_put_contents_dup(dir::appdata("spa_install.xml"), node.GetXML(nullptr));
      }
install:
...
/*      win::thread * pthread = dynamic_cast < win::thread *> (papp->pointer < ::ca::thread > ::m_p);
  ...
  /*    ::SetEvent(m_hEventReady);
   ...
   //   pplugin->m_pframe = dynamic_cast < bergedge::frame * > (pplugin->get_context_application()->get_edge(0, true)->get_bergedge_view()->GetParentFrame());
   ...
   /*   papp->m_puiInitialPlaceHolderContainer = pplugin->m_pframe;
      ...
      pplugin->m_pframe->GetParent()->on_layout(::draw2d::graphics_pointer & pgraphics);*/
