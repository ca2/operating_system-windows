#include "framework.h"


#ifdef _X86_
#pragma function(__memcmp)
::i32 __memcmp(const void * m1, const void * m2, size_t s)
{
   return __memcmp((void *) m1, (void *) m2, s);
}
#pragma function(memory_set)
void * memory_set( void *_dst, ::i32 _val, size_t _sz )
{
   return memory_set(_dst, _val, _sz);
}
#else
/*void * memory_set(void * p, ::i32 uch, size_t iSize)
{
   return memory_set(dst, src, iSize);
}
*/
#pragma function(__memcmp)
::i32 __memcmp(const void * m1, const void * m2, size_t s)
{
   return __memcmp((void *) m1, (void *) m2, s);
}
#pragma function(memory_set)
void * memory_set( void *_dst, ::i32 _val, size_t _sz )
{
    ::u8 * puch = (::u8 * ) _dst;
   while(_sz > 0)
   {
      *puch = static_cast < ::u8 > (_val);
      puch++;
      _sz--;
   }
   return _dst;
}

/*#pragma function(calloc)
void * calloc(  size_t sz, size_t sz2 )
{
   return ca2_alloc(sz * sz2);
}*/

#pragma function(memory_copy)
void * memory_copy(void * dst, const void * src, memsize iSize)
{
   return memory_copy(dst, src, iSize);
}

#endif
