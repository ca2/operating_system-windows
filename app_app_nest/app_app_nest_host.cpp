...
namespace ca2plugin_container
{
   ...
   host::host(::pointer<::ca2plugin_container::application>papp) :
      ::object(papp),
...
//         else if(message == ::hotplugin::message_set_window)
         if(message == ::hotplugin::message_set_window)
         {
            ...
            //if(m_rectangle != rectangle)
            {
               ...
               m_rectangleX.offset(-m_rectangleX.top_left());
         ...
         else if(message == ::hotplugin::message_paint)
         {
            ...
            const ::i32_rectangle & rectangle = *((LPCRECT) pdata);
            ...
            try
            {
               ...
               on_paint(nullptr,i32_rectangle);
   ...
   bool host::set_window_position(iptr z,int x,int y,int cx,int cy,UINT nFlags)
   {
      ...
      try
      {
         ...
         if(m_pcontainerapp != nullptr)
         {
            ...
            if(m_pcontainerapp->system() != nullptr)
            {

               if(m_pcontainerapp->system()->m_pplugin != nullptr)
               {

                  if(m_pcontainerapp->system()->m_pplugin->m_puserinteractionHost != nullptr)
                  {

                     m_pcontainerapp->system()->m_pplugin->m_puserinteractionHost->set_window_position(z,x,y,cx,cy,nFlags);
