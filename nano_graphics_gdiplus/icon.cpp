...
HBITMAP LoadPNGFromMemoryIntoHBITMAP(const void *pngData, size_t dataSize, HDC hdc)
{
   ...
   hr = pFactory->CreateDecoderFromStream(pStream, // Stream with the PNG data
   ...
   hr = pConverter->Initialize(pFrame, // Input frame
   ...
   hr = pConverter->CopyPixels(NULL, // Copy the entire bitmap
