#include "framework.h"


//namespace multimedia
//{
//
//
//   namespace audio_mixer_mmsystem
//   {
//
//
//      factory_exchange::factory_exchange(::object * pobject) :
//         ::object(pobject),
//         ::multimedia::mmsystem::factory_exchange(pobject)
//      {
//
//         //create_factory < callback, ::multimedia::audio_mixer::callback >();
//         create_factory <control, ::multimedia::audio_mixer::control>();
//         //create_factory <control_data                >  (__type(::multimedia::audio_mixer::control_data));
//         create_factory <destination,::multimedia::audio_mixer::destination>();
//         create_factory <device,::multimedia::audio_mixer::device>();
//         create_factory <line_info,::multimedia::audio_mixer::line_info>();
//         create_factory < audio_mixer, ::multimedia::audio_mixer::audio_mixer >();
//         //create_factory < department, ::multimedia::audio_mixer::font >();
//         create_factory <source,::multimedia::audio_mixer::source>();
//         create_factory <thread,::multimedia::audio_mixer::thread>();
//         /*create_factory <user::label                 >  (__type(::multimedia::audio_mixer::user::label));
//         create_factory <user::interaction               >  (__type(::multimedia::audio_mixer::user::control));
//         create_factory <user::level_control         >  (__type(::multimedia::audio_mixer::user::level_control));
//         create_factory <user::toggle_control        >  (__type(::multimedia::audio_mixer::user::toggle_control));*/
//
//      }
//
//
//      factory_exchange::~factory_exchange()
//      {
//
//      }
//
//
//   } // namespace audio_mmsystem
//
//
//} // namespace multimedia

extern "C"
void audio_mixer_mmsystem_factory_exchange(::factory_map * pfactorymap)
{

            //create_factory < callback, ::multimedia::audio_mixer::callback >();
   pfactorymap->create_factory <::multimedia::audio_mixer_mmsystem::control, ::multimedia::audio_mixer::control>();
            //create_factory <control_data                >  (__type(::multimedia::audio_mixer::control_data));
   pfactorymap->create_factory <::multimedia::audio_mixer_mmsystem::destination,::multimedia::audio_mixer::destination>();
   pfactorymap->create_factory <::multimedia::audio_mixer_mmsystem::device,::multimedia::audio_mixer::device>();
   pfactorymap->create_factory <::multimedia::audio_mixer_mmsystem::line_info,::multimedia::audio_mixer::line_info>();
   pfactorymap->create_factory <::multimedia::audio_mixer_mmsystem::audio_mixer, ::multimedia::audio_mixer::audio_mixer >();
            //create_factory < department, ::multimedia::audio_mixer::font >();
   pfactorymap->create_factory <::multimedia::audio_mixer_mmsystem::source,::multimedia::audio_mixer::source>();
   pfactorymap->create_factory <::multimedia::audio_mixer_mmsystem::thread,::multimedia::audio_mixer::thread>();
            /*create_factory <user::label                 >  (__type(::multimedia::audio_mixer::user::label));
            create_factory <user::interaction               >  (__type(::multimedia::audio_mixer::user::control));
            create_factory <user::level_control         >  (__type(::multimedia::audio_mixer::user::level_control));
            create_factory <user::toggle_control        >  (__type(::multimedia::audio_mixer::user::toggle_control));*/

}



