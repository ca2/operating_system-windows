...
namespace hi5
{
   ...
   *--*/
   bool facebook::performGet(const string & getUrl)
   {
      ...
      /* Send http request */
      return pcontext->m_pcontext->http().get(getUrl, m_strResponse, set);
   ...
   *--*/
   bool facebook::performDelete(const string & deleteUrl)
   {
      ...
      /* Send http request */
      return pcontext->m_pcontext->http().get(deleteUrl, m_strResponse, set);
   ...
   *--*/
   bool facebook::performPost(const string & postUrl, property_set & set)
   {
      ...
      if (set.has_property("headers"))
      {
         ...
         return pcontext->m_pcontext->http().get(postUrl, m_strResponse, set);
      ...
      else
      {
         ...
         return pcontext->m_pcontext->http().get(postUrl, m_strResponse, setHttp) && setHttp["get_status"].int32() == ::success;
